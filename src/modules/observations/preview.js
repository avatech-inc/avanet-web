...
const ObservationPreview = [
    ...
    (
        ...
        $scope.showMedia = media => {
            // clear audio
            let nonAudioMedia = []

            angular.forEach($scope.observation.media, (m) => {
                if (m.type === 'photo' || m.type === 'video') {
                    nonAudioMedia.push(m)
                }
            })

            // get index
            let index = nonAudioMedia.indexOf(media)

            // show media
            if (media.type === 'photo' || media.type === 'video') {
                Lightbox.openModal(nonAudioMedia, index)
        ...
        $scope.delete = () => {
            Confirm.open('Are you sure you want to delete this observation? ' +
                'This will also delete it from your Avanet mobile apps.')
            .then(() => {
        ...
        $scope.getTriggerNames = triggerCodes => {
            let names = ''

            if (!triggerCodes || !triggerCodes.length) return ''

            for (let i = 0; i < triggerCodes.length; i++) {
                let _name = $scope.getTriggerName(triggerCodes[i])
        ...
        $scope.getView = () => {
            ...
            angular.forEach($scope.views, view => {
                if (view.id === $scope.settings.view) {
                    _view = view
                }
            })
        ...
        $scope.exportPDF = () => snowpitExport.PDF(  // eslint-disable-line new-cap
            $scope.observation,
            $scope.settings
        )

        $scope.exportJPEG = () => snowpitExport.JPEG(  // eslint-disable-line new-cap
            $scope.observation,
            $scope.settings
        )

        $scope.exportCSV = () => snowpitExport.CSV(  // eslint-disable-line new-cap
            $scope.observation
        )
        ...
        $scope.getTable = ob => {
            ...
            let table = []
            let items = schema['observation_' + ob.type]

            if (items) {
                for (let i = 0; i < items.length; i++) {
                    let key = items[i][0]
                    let desc = items[i][1]
                    let dataType = items[i][2]
        ...
        if ($stateParams.observationId) {
            ...
            $http
                .get(window.apiBaseUrl + 'observations/' + $stateParams.observationId)
                .then(res => {
                    let ob = res.data
