angular.module('avatech').controller('AvalanchePreviewController', [ '$scope', '$rootScope', '$location', '$stateParams', '$state', 'Tests', 'FontLoader', 'Global', 'Restangular', 'snowpitExport', 'Lightbox',
    function ($scope, $rootScope, $location, $stateParams, $state, Tests, FontLoader, Global, Restangular, snowpitExport, Lightbox) {
    ...
    $scope.naturalTriggers = [
        ...
        if ($stateParams.observationId) {
            ...
            Restangular.one('observations',$scope.observationId).get().then(function(observation) {

                // broadcast so map can show location
                $rootScope.$broadcast('profileLoaded', observation);
