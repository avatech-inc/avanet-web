...
const Uploader = [
    ...
    ($http, $timeout, $log) => ({
        ...
        templateUrl: '/js/directives/fileupload.html',
        link: (scope, element) => {
            let filesUpload = element[0].querySelector('input')
            let dropArea = element[0].querySelector('.upload-area')
            ...
            let uploadFile = file => {
                ...
                xhr.upload.addEventListener('progress', function (evt) {
                    if (evt.lengthComputable) {
                        ...
                        if (progress > fileObject.progress) {
                        ...
                        if (scope.onprogress) {
                            scope.onprogress({ file: fileObject })
                        }
                    }
                }, false)
                ...
                xhr.onreadystatechange = () => {
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if (status === 200) {
                            ...
                            if (scope.onupload) {
                                scope.onupload({ file: fileObject })
                            } else {
                ...
                //   //   fileObject.progress = 20; if (scope.onprogress)
                // scope.onprogress({ file: fileObject });
                //   //   return function (evt) {
            ...
            //   fileObject.progress = 30; if (scope.onprogress)
            // scope.onprogress({ file: fileObject });
            ...
            //   fileObject.progress = 40; if (scope.onprogress)
            // scope.onprogress({ file: fileObject });
            ...
            //   fileObject.progress = 50; if (scope.onprogress)
            // scope.onprogress({ file: fileObject });
            //   // // resize
            ...
            //   //setTimeout(function(){
            //   fileObject.progress = 60; if (scope.onprogress)
            // scope.onprogress({ file: fileObject });
            //   //},10);
